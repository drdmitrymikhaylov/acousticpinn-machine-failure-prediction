...
phase-locked to the mains and filtered by the propagation path. Each is
checked against an independent prediction before it is used (see *Checks
that ran*).
...
|---|------|---------------------|
| 1 | Never look for a defect frequency in the raw spectrum. Demodulate. | Exp 1: detection rate 0.24 vs 1.00 at -12 dB |
| 2 | Sum no more than `k_max ≈ 0.225 / slip` harmonics. At 1% slip that is 22; at 4% it is 6. | Exp 2, verified to 3% over a 16× range of slip |
...
## Exp 1 Demodulation: the defect is not where the energy is
...
the structural resonance, several kilohertz up. The defect frequency appears
only as the *repetition rate* of that ringing, i.e. as a modulation.
...
The middle panel is the kurtogram: the kurtosis of the band-limited envelope
over a grid of centre frequencies and bandwidths. Impulsive content raises it,
so the brightest cell should be the band the impacts live in. It finds the
true 8 kHz resonance here. Exp 3 asks whether that is worth anything.
...
search range. Any statistic measured against a global noise floor then
reports a defect that is not there. This is the kind of bug that ships.

---

## Exp 2 Slip: how far up the harmonic series you can go
...
decay time (~1 ms here). That alone rolls the harmonic series off with a
corner near 160 Hz, below the second harmonic. Fitting the jitter law without
dividing that out returns σ ~ 0.017 for every slip from 0.25% to 4%. It
measures the impulse shape and calls it slip.

**Random-walk slip.** If instead the *interval* fluctuates, phase error
accumulates and the lines broaden rather than fade. The measured exponent of
linewidth against harmonic order is **1.98, 2.01, 2.00** for slips of 0.25%,
0.5% and 1%, which is the predicted k². At 2% and 4% the fit falls to 1.64
and 1.12, because neighbouring harmonics have merged and there is no separate
line left to measure.
...
## Exp 3 Band selection: the result that was not expected
...
Adaptive band selection **is no better than the fixed wide band**. On a white
...
It is **significantly behind the fixed band at every SNR from 0 to -12 dB**
(8–10 records of 90 behind, p = 0.003–0.03), because the interferer is
impulsive enough to attract the kurtosis maximum.

Every rate in Exp 3 is pooled over three defect types (outer race, inner
race, rolling element), 30 records each, so 90 records per cell. With the
interval attached (`results/band_selection_intervals.json`, Wilson 95 %):

| SNR | background | fixed 2–20 kHz | kurtogram | oracle | kurtogram minus fixed | oracle minus fixed |
|---|---|---|---|---|---|---|
...
outer race, inner race and rolling element; the kurtogram detects 15, 18
and 19.
...
the resonance**, which is a fixed property of the sensor and its mounting. It
...
## Exp 4 Partial discharge, and the loose bolt that looks exactly like it
...
Below about -12 dB the count is a **lower bound, not a measurement**. Every
statistic built on top of it (rate, phase concentration, Fano factor)
...
audio-band energy is what a threshold on a broadband ultrasonic meter amounts
to. It separates discharge from sensor noise at AUC 0.36–0.55, i.e. not at
all, and at the low end *backwards*. Broadband sensor noise has a flatter
spectrum than a discharge that has crossed oil and steel, so the ratio can be
higher for a healthy sensor than for a live fault. What separates them is the
phase-locked event structure, which no energy ratio can see.

**Limit of this section.** Against the mechanical confuser, the six features
separate discharge at AUC ~ 1.0 at every SNR tested. But so does the crest
factor on its own, and nearly so does the energy ratio. That says the model's
two classes are further apart than plant classes are, not that the problem is
solved. The transferable results here are the counting calibration and the
negative result about energy ratios; the AUC is not.

---

## Exp 5 Alarm policy: from a statistic to an alarm somebody will act on
...
quiet history and then 48 h in which the defect grows. It sets the threshold
from the first half of each machine's *own* history and measures false
alarms on the held-out half:
...
## Exp 6 Resonance identification from the record itself

![resonance identification](figures/06_resonance_identification.png)

Exp 3 ended with a configuration step: measure each sensor's structural
...
They are high-passed above 1 kHz, so that the shaft harmonics do not sit
inside a window as an offset and a slope. Three estimators are asked for
(ωₙ, ζ):
...
- **PINN** — a network x(t) for all windows, with a window embedding and
  Fourier features. Its loss is the data misfit plus the residual of the
  equation of motion at collocation points between the samples, with (ωₙ, ζ)
  trainable and shared across windows.

The band for demodulation is then centred on the identified resonance and
run through the Exp 3 detection benchmark against the fixed band, the
kurtogram and the oracle. The benchmark uses 12 machines per SNR, resonances
drawn between 4 and 14 kHz, and damping 0.01–0.05.
...
not. Below that the per-window fit collapses, to an 847 Hz median and a
damping error of 550 % at -15 dB. The joint fit, which has sixteen windows'
...
difference between AUC 0.90 and 0.91 at -15 dB, both close to the oracle's
1.00. At these SNRs the fixed 2–20 kHz band also detects perfectly, and the
value of a resonance estimate is the one Exp 3 measured at -18 dB.

**The PINN result is negative.** It is the worst of the three estimators at
every SNR (median errors of 0.5–3 kHz below -9 dB, AUC 0.66–0.82), and the
reasons are recorded because they are general:
...
The rule this gives the framework is simple. When the physics is a
two-parameter equation and the data are short windows of it, make the physics
the family of functions being fitted (a damped sinusoid with shared
parameters) rather than a loss term on a free function. The PINN belongs where the field is not
known in closed form; here it is.

**What this changes in rule 4.** The commissioning impact test remains the
right thing to do. It costs a minute and needs no algorithm. But where it was
not done, a joint free-decay fit over the strongest impacts of a faulty record
recovers the resonance to within 100 Hz at -15 dB. That is enough to place
the demodulation band; the record can bootstrap its own oracle.
...
- kinematic frequencies reproduce the tabulated multipliers for a documented
  bearing geometry to 2 × 10⁻⁵. This included resolving an apparent
  factor-of-two discrepancy in the published ball-spin figure, which lists
  the *impact* rate (a spall strikes both races), not the ball rotation rate
...
- **the Exp 6 result is pinned**: at every SNR the joint fit is at least as
...
- **The discharge classes are too far apart**, as Exp 4 says of itself.

---

## Build log

The Exp 3 text was corrected once. The first version said the kurtogram was
"at or below the fixed band everywhere" on a white background. When I went
back and attached the sample size to every rate (90 records per cell, three
defect types times 30), the -18 dB cell read 52 against 42 in the kurtogram's
favour, p = 0.14. That is inside the noise, but it is not "at or below". The
interferer rows held up: 8–10 records behind at every SNR from 0 to -12 dB.
...
loopholes were closed was the poor result taken as real. The full Exp 6 run
takes about 2 h on a laptop CPU. The kinematics test also caught a
factor-of-two disagreement with a published ball-spin figure, which turned out
to be the impact rate rather than the ball rotation rate.
...
- `src/exp2_detection.py`: the detection benchmark of Exp 3 (fleet of
  machines, band strategies, pooled scoring)
- `src/exp5_resonance_pinn.py`: the three resonance estimators of Exp 6 and
  the benchmark that compares them
- `src/band_selection_intervals.py`: reads the Exp 3 result files and
  attaches counts, Wilson intervals and two-proportion tests to every rate
...
The three regimes covered are rotating machinery, partial discharge in
high-voltage plant, and threshold policy for continuous monitoring. They are
the ones where the acoustic signature has a mechanism you can write down and
